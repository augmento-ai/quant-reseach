...
all_data = eh.load_example_data(filename_augmento_topics, filename_augmento_data, filename_bitmex_data)
...
sent_score = ah.nb_calc_sentiment_score_c(aug_signal_a, aug_signal_b, 28*24, 14*24)

date_time = np.array([datetime.datetime.utcfromtimestamp(t).isoformat()
 for t in t_price_data])
...
csv_data= {"date": date_time, "ref_price":price_data,"raw_signal":sent_score,"pnl":pnl}

data_frame = pd.DataFrame(csv_data)

data_frame.to_csv("data/signals.csv",index=False)
